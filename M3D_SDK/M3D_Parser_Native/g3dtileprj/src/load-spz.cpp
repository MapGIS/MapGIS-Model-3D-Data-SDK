...
template<typename T>
const T& user_clamp(const T& v, const T& lo, const T& hi) {
	// 断言：保证下界 <= 上界，否则行为未定义（与 std::clamp 一致）
	// 可选：使用 assert 或 static_assert（如果能在编译时判断）
...
uint8_t toUint8(float x) { return static_cast<uint8_t>(user_clamp(std::round(x), 0.0f, 255.0f)); }
...
uint8_t quantizeSH(float x, int32_t bucketSize) {
  ...
  return static_cast<uint8_t>(user_clamp(q, 0, 255));
}
...
{
	float epsilon = 1e-6f; // 例如 0.000001
	if (x < epsilon)
		x = epsilon;
	else if (x > 1.0f - epsilon)
		x = 1.0f - epsilon;
...
void packQuaternionSmallestThree(uint8_t r[4], const float rotation[4], const MapGIS::Tile::CoordinateSystemConverter& c) {
  // Normalize the quaternion
  Quat4f q = normalized(quat4f(&rotation[0]));

  std::array<float, 4> newRotation = MapGIS::Tile::CGGaussianDataTrans::TransRotations({ q[0],q[1],q[2],q[3] }, c.match);
  q[0] = newRotation[0];
  q[1] = newRotation[1];
  q[2] = newRotation[2];
  q[3] = newRotation[3];
...
PackedGaussians packGaussians(const GaussianCloud &g, const PackOptions &o) {
  ...
  MapGIS::Tile::CoordinateSystemConverter c = MapGIS::Tile::CGGaussianDataTrans::GetCoordinateSystemConverter(o.from, MapGIS::Tile::CoordinateSystem::RBU);
  ...
  for (size_t i = 0; i < numPoints ; i++)
  {
	  ...
	  std::array<float, 3> rtn = MapGIS::Tile::CGGaussianDataTrans::TransPoints({ g.positions[i*3] * scale ,g.positions[i * 3 +1] * scale ,g.positions[i * 3 + 2] * scale }, c.match);
	
	  int32_t fixed32 = static_cast<int32_t>(std::round(rtn[0]));
	  packed.positions[i * 9 + 0] = fixed32 & 0xff;
	  packed.positions[i * 9 + 1] = (fixed32 >> 8) & 0xff;
	  packed.positions[i * 9 + 2] = (fixed32 >> 16) & 0xff;

	  fixed32 = static_cast<int32_t>(std::round(rtn[1]));
	  packed.positions[i * 9 + 3] = fixed32 & 0xff;
	  packed.positions[i * 9 + 4] = (fixed32 >> 8) & 0xff;
	  packed.positions[i * 9 + 5] = (fixed32 >> 16) & 0xff;

	  fixed32 = static_cast<int32_t>(std::round(rtn[2]));
	  packed.positions[i * 9 + 6] = fixed32 & 0xff;
	  packed.positions[i * 9 + 7] = (fixed32 >> 8) & 0xff;
	  packed.positions[i * 9 + 8] = (fixed32 >> 16) & 0xff;
  }


  for (size_t i = 0; i < numPoints; i++)
  {
	  std::array<float, 3> rtn = MapGIS::Tile::CGGaussianDataTrans::TransScale({ (g.scales[i * 3] + 10.0f) * 16.0f ,(g.scales[i * 3 + 1] + 10.0f) * 16.0f ,(g.scales[i * 3 + 2] + 10.0f) * 16.0f }, c.match);
	  packed.scales[i * 3] = toUint8(rtn[0]);
	  packed.scales[i * 3 + 1] = toUint8(rtn[1]);
	  packed.scales[i * 3 + 2] = toUint8(rtn[2]);
  }
  ...
  if (g.shDegree > 0) {
	...
	for (size_t i = 0; i < numPoints; i++) {
		std::vector<float> tempSh;
		tempSh.insert(tempSh.begin(), g.sh.begin() + i *shPerPoint, g.sh.begin() + (i + 1) *shPerPoint);

		std::vector<float> newSh = MapGIS::Tile::CGGaussianDataTrans::TransSh(tempSh, c.match);
		int j = 0;

		for (; j < 9; j++) {  // There are 9 (3 * 3) coefficients for degree 1
			packed.sh[i *shPerPoint + j] = quantizeSH(newSh[j], 1 << (8 - sh1Bits));
		}

		for (; j < shPerPoint; j++) {
			packed.sh[i *shPerPoint + j] = quantizeSH(newSh[j], 1 << (8 - shRestBits));
		}
...
void unpackQuaternionFirstThree(float rotation[4], const uint8_t r[3], const MapGIS::Tile::CoordinateSystemConverter& c = MapGIS::Tile::CoordinateSystemConverter())
{
	Vec3f xyz = plus(times(Vec3f{ static_cast<float>(r[0]), static_cast<float>(r[1]), static_cast<float>(r[2]) }, 1.0f / 127.5f), Vec3f{ -1, -1, -1 });
	float w = std::sqrt(std::max(0.0f, 1.0f - squaredNorm(xyz)));
	std::array<float, 4> newRotation = MapGIS::Tile::CGGaussianDataTrans::TransRotations({ xyz[0],xyz[1],xyz[2],w }, c.match);
	rotation[0] = newRotation[0];
	rotation[1] = newRotation[1];
	rotation[2] = newRotation[2];
	rotation[3] = newRotation[3];
}

void unpackQuaternionSmallestThree(float rotation[4], const uint8_t r[4], const MapGIS::Tile::CoordinateSystemConverter& c = MapGIS::Tile::CoordinateSystemConverter())
{
  uint32_t comp =
    r[0] +
    (r[1] << 8) +
  ...
  std::array<float, 4> newRotation = MapGIS::Tile::CGGaussianDataTrans::TransRotations({ rotation [0],rotation [1],rotation [2],rotation [3]}, c.match);
  rotation[0] = newRotation[0];
  rotation[1] = newRotation[1];
  rotation[2] = newRotation[2];
  rotation[3] = newRotation[3];
...
UnpackedGaussian PackedGaussian::unpack(
  bool usesFloat16, bool usesQuaternionSmallestThree, int32_t fractionalBits, const MapGIS::Tile::CoordinateSystemConverter &c) const {
  ...
  if (usesFloat16) {
	...
	{
		std::array<float, 3> rtn = MapGIS::Tile::CGGaussianDataTrans::TransPoints({ halfToFloat(halfData[0]),halfToFloat(halfData[1]) ,halfToFloat(halfData[2]) }, c.match);
		result.position[0] = rtn[0];
		result.position[1] = rtn[1];
		result.position[2] = rtn[2];
	}
    ...
    for (size_t i = 0; i < 3; i++) {
	  ...
	  value[i] =  static_cast<float>(fixed32) * scale;
    }

	std::array<float, 3> rtn = MapGIS::Tile::CGGaussianDataTrans::TransPoints(value, c.match);
	result.position[0] = rtn[0];
	result.position[1] = rtn[1];
	result.position[2] = rtn[2];
  }
  ...
  std::array<float, 3>  scales = MapGIS::Tile::CGGaussianDataTrans::TransScale({ result.scale[0] ,result.scale[1],result.scale[2] }, c.match);
  result.scale[0] = scales[0];
  result.scale[1] = scales[1];
  result.scale[2] = scales[2];
  ...
  for (size_t i = 0; i < 15; i++) {
	  sh.push_back(unquantizeSH(shR[i]));
	  sh.push_back(unquantizeSH(shG[i]));
	  sh.push_back(unquantizeSH(shB[i]));
  }
  std::vector<float> newSh = MapGIS::Tile::CGGaussianDataTrans::TransSh(sh, c.match);

  for (size_t i = 0; i < 15; i++) {
	  result.shR[i] = newSh[i * 3];
	  result.shG[i] = newSh[i * 3 +1];
	  result.shB[i] = newSh[i * 3+1];
...
UnpackedGaussian PackedGaussians::unpack(int32_t i, const MapGIS::Tile::CoordinateSystemConverter &c) const {
...
GaussianCloud unpackGaussians(const PackedGaussians &packed, const UnpackOptions &o) {
  ...
  result.convertCoordinates(MapGIS::Tile::CoordinateSystem::RUF, o.to);
  return result;
}
...
GaussianCloud loadSplatFromPly(const std::string &filename, const UnpackOptions &o) {
  ...
  result.convertCoordinates(MapGIS::Tile::CoordinateSystem::RBU, o.to);
  return result;
}
...
bool saveSplatToPly(const GaussianCloud &data, const PackOptions &o, const std::string &filename) {
  ...
  MapGIS::Tile::CoordinateSystemConverter c = MapGIS::Tile::CGGaussianDataTrans::GetCoordinateSystemConverter(o.from, MapGIS::Tile::CoordinateSystem::RDF);
  ...
  for (int32_t i = 0; i < N; i++)
  {
	  // Position (x, y, z)
	  {
		  std::array<float, 3> rtn = MapGIS::Tile::CGGaussianDataTrans::TransPoints({ data.positions[i3 + 0] ,data.positions[i3 + 1] ,data.positions[i3 + 2] }, c.match);
		  values[outIdx++] = rtn[0];
		  values[outIdx++] = rtn[1];
		  values[outIdx++] = rtn[2];
	  }
	  ...
	  { // Spherical harmonics: Interleave so the coefficients are the fastest-changing axis and
		// the channel (r, g, b) is slower-changing axis.
		  ...
		  std::vector<float> oldSh;
		  oldSh.insert(oldSh.end(), data.sh.begin() + shDim * 3 * i, data.sh.begin() + shDim * 3 * (i + 1));
		 
		  for (int32_t j = 0; j < shDim; j++) {
			  values[outIdx++] = oldSh[j * 3];
		  }
		  for (int32_t j = 0; j < shDim; j++) {
			  values[outIdx++] = oldSh[j * 3+1];
		  }

		  for (int32_t j = 0; j < shDim; j++) {
			  values[outIdx++] = oldSh[j * 3+2];
		  }
	  }
	  ...
	  // Scale (sx, sy, sz)
	  {
		  std::array<float, 3> rtn = MapGIS::Tile::CGGaussianDataTrans::TransScale({ data.scales[i3 + 0], data.scales[i3 + 1] ,data.scales[i3 + 2] }, c.match);
		  values[outIdx++] = rtn[0];
		  values[outIdx++] = rtn[1];
		  values[outIdx++] = rtn[2];
	  }
	  ...
	  {
		  std::array<float, 4> rtn = MapGIS::Tile::CGGaussianDataTrans::TransRotations({ data.rotations[i4 + 0] , data.rotations[i4 + 1] , data.rotations[i4 + 2], data.rotations[i4 + 3] }, c.match);
		  values[outIdx++] = rtn[3];
		  values[outIdx++] = rtn[0];
		  values[outIdx++] = rtn[1];
		  values[outIdx++] = rtn[2];
