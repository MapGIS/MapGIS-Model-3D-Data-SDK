...
class JsonWriter {
 public:
  ...
  void OutputValue(const int8_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }

  void OutputValue( const int16_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }

  void OutputValue( const int32_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }

  void OutputValue( const int64_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }
  ...
  void OutputValue( const uint8_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }

  void OutputValue(const uint16_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }

  void OutputValue( const uint32_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }

  void OutputValue( const uint64_t &value) {
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << std::setprecision(17) << std::to_string(value);
  }
  ...
  void OutputValue(const std::string &name, const int8_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }

  void OutputValue(const std::string &name, const int16_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }

  void OutputValue(const std::string &name, const int32_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }

  void OutputValue(const std::string &name, const int64_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }
 
  void OutputValue(const std::string &name, const float &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }

  void OutputValue(const std::string &name, const double &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }
  ...
  void OutputValue(const std::string &name, const uint8_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }

  void OutputValue(const std::string &name, const uint16_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }

  void OutputValue(const std::string &name, const uint32_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }

  void OutputValue(const std::string &name, const uint64_t &value) {
	  const std::string escaped_name = EscapeJsonSpecialCharacters(name);
	  FinishPreviousLine(VALUE);
	  o_ << indent_ << "\"" << escaped_name << "\":" << separator_ << std::to_string(value);
  }
