...
namespace Microsoft
{
    namespace glTF
    {
        namespace Detail
        {
			...
			struct Colors
			{
    			static TColor DIELECTRIC_SPECULAR;
    			static TColor BLACK;
			};
            template<typename TColor>
            TColor Colors<TColor>::DIELECTRIC_SPECULAR = TColor(0.04f, 0.04f, 0.04f);

            template<typename TColor>
            TColor Colors<TColor>::BLACK = TColor(0.0f, 0.0f, 0.0f);
		#endif
		...
		#else
			const float dielectricSpecularRed = R(Colors<TColor>::DIELECTRIC_SPECULAR);
		#endif
		...
		#else
            const TColor baseColorFromSpecular = (sg.specular - (Colors<TColor>::DIELECTRIC_SPECULAR) * (oneMinusMetallic)) * (1.0f / std::max(metallic, std::numeric_limits<float>::epsilon()));
		#endif
		...
		#else
            const auto specular = TColor::Lerp(Colors<TColor>::DIELECTRIC_SPECULAR, mr.base, mr.metallic);
            const float oneMinusSpecularStrength = 1.0f - GetMaxComponent(specular);
            const auto diffuse = oneMinusSpecularStrength < std::numeric_limits<float>::epsilon()
                ? Colors<TColor>::BLACK
                : mr.base * ((1.0f - R(Colors<TColor>::DIELECTRIC_SPECULAR)) * (1.0f - mr.metallic) / oneMinusSpecularStrength);
		#endif
